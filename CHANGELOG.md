...
### Added
- `pruneSessionCaches` RPC handler (new upstream method in v1.12603.0, called by Desktop's VM disk janitor every 300s, before spawns when disk is low, and from the manual disk cleanup menu) - routed through the `VMBackend` interface: the native backend is a typed no-op returning `{prunedSessions: [], skippedSessions: [], freedBytes: 0, errors: {}}` (native sessions have no VM-style caches; the CLI's caches live in the user's real home and must not be touched), the KVM backend forwards it to the guest sdk-daemon which does the actual pruning inside the VM image
